  ...
  return (
    <section style={{ minHeight: '100vh', background: tc.heroBg, position: 'relative', display: 'flex', alignItems: 'center', overflow: 'hidden', paddingTop: '72px' }}>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: `${MESSIKA_SPACING['5xl']} 48px`, width: '100%', display: 'grid', gridTemplateColumns: '55% 45%', gap: '48px', alignItems: 'center', position: 'relative', zIndex: 5 }} className="hero-grid">
        ...
        {/* Right — decorative mini-card grid */}
        <motion.div style={{ y: shouldReduce ? 0 : yOffset }} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9, delay: 0.3 }} className="hero-visual">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3px', transform: 'perspective(800px) rotateY(-8deg) rotateX(4deg)', transformStyle: 'preserve-3d' }}>
            {(offers.slice(0, 6).length > 0 ? offers.slice(0, 6) : Array(6).fill(null)).map((offer, i) => (
              <motion.div key={i} initial={shouldReduce ? {} : { opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + i * 0.08 }} onClick={() => setCurrentPage('offers')}
                style={{ height: i < 2 ? '180px' : '140px', background: tc.cardBg, overflow: 'hidden', cursor: 'pointer', position: 'relative' }}>
                {offer?.image
                  ? <img src={offer.image} alt={offer.name} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                  : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: tc.imageBg, fontSize: '32px', opacity: 0.5 }}>💎</div>}
                <div style={{ position: 'absolute', inset: 0, background: tc.imgOverlay }} />
                {offer && <div style={{ position: 'absolute', bottom: 8, left: 10, fontFamily: MESSIKA_FONTS.display, fontSize: '11px', fontWeight: 400, color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '90%' }}>{offer.name}</div>}
              </motion.div>
            ))}
          </div>
        </motion.div>
